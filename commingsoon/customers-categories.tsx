...
// Dummy data for the customer categories table
const dummyCustomerCategories = [
  { id: 1, name: "Regular", code: "REG", description: "Regular customers", status: "active" },
  { id: 2, name: "VIP", code: "VIP", description: "VIP customers with special privileges", status: "active" },
  { id: 3, name: "Corporate", code: "CORP", description: "Business/corporate customers", status: "inactive" },
  { id: 4, name: "Wholesale", code: "WHSL", description: "Wholesale customers", status: "active" },
  { id: 5, name: "New", code: "NEW", description: "New customers", status: "inactive" },
];

function RouteComponent() {
  const [customerCategories, setCustomerCategories] = useState(dummyCustomerCategories);
  ...
  // Filter customer categories based on search query
  const filteredCategories = customerCategories.filter(category => 
  ...
  const handleDeleteCategory = (id: number) => {
    if (confirm("Are you sure you want to delete this customer category?")) {
      setCustomerCategories(customerCategories.filter(category => category.id !== id));
    }
  };
  ...
  const validateName = (name: string): boolean => {
    ...
    if (!isEditing && customerCategories.some(category => category.name.toLowerCase() === name.toLowerCase())) {
    ...
    if (isEditing && customerCategories.some(category => category.id !== currentCategory.id && category.name.toLowerCase() === name.toLowerCase())) {
      setNameError("This category name already exists");
      return false;
    }
  ...
  const handleSaveCategory = () => {
    ...
    if (isEditing) {
      setCustomerCategories(customerCategories.map(category => category.id === currentCategory.id ? currentCategory : category));
    } else {
      const newId = Math.max(...customerCategories.map(category => category.id)) + 1;
      setCustomerCategories([...customerCategories, { ...currentCategory, id: newId }]);
    }
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-4">Customer Categories Management</h1>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{isEditing ? "Edit Customer Category" : "Add New Customer Category"}</DialogTitle>
            <DialogDescription>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                className="w-full"
                placeholder="Brief description of the customer category"
              />
            </div>
            <div className="grid gap-2">
